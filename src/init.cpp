...
//    strUsage += HelpMessageOpt("-spentindex", strprintf(_("Maintain a full spent index, used to query the spending txid and input index for an outpoint (default: %u)"), DEFAULT_SPENTINDEX));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 2019, 12019));
    ...
    strUsage += HelpMessageOpt("-anonymizedarkamount=<n>",strprintf(_("Keep N CMM anonymized (default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all masternode specific functionality (Masternodes, SwiftX, Budgeting) (0-1, default: %u)"), 0));
...
)
{
    ...
    if (!(
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("Cannot find the shielding parameters in the following directory:\n"
              "%s\n"
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    nSendRounds = GetArg("-darksendrounds", 10);
    if (nSendRounds > 100) nSendRounds = 100;
    if (nSendRounds < 10) nSendRounds = 10;
    ...
    nAnonymizeDarkAmount = GetArg("-anonymizedarkamount", 0);
    if (nAnonymizeDarkAmount > 999999) nAnonymizeDarkAmount = 999999;
    if (nAnonymizeDarkAmount < 2) nAnonymizeDarkAmount = 2;
    ...
    LogPrintf("Dark send rounds %d\n", nSendRounds);
    LogPrintf("Anonymize Dark Amount %d\n", nAnonymizeDarkAmount);
    /* Denominations
       A note about convertability. Within Obfuscation pools, each denomination
       is convertable to another.
       For example:
       1CMM+1000 == (.1CMM+100)*10
       10CMM+10000 == (1CMM+1000)*10
    */
