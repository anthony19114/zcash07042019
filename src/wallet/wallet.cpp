...
int CWallet::GetInputObfuscationRounds(CTxIn in) const
{
    ...
    return realObfuscationRounds > nSendRounds ? nSendRounds : realObfuscationRounds;
}
    ...
    CAmount nTotal = 0;
    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                int rounds = GetInputObfuscationRounds(vin);
                nTotal += pcoin->vout[i].nValue * rounds / nSendRounds;
            }
        }
    }
    return nTotal;
    ...
    if (coinControl && coinControl->HasSelected() && !coinControl->fAllowOtherInputs)
    {
        BOOST_FOREACH(const COutput& out, vCoins)
        {
            ...
            if (coin_type == ONLY_DENOMINATED) {
                ...
                if (rounds < nSendRounds) continue;
            }
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, fOnlyCoinbaseCoins, fNeedCoinbaseCoins, coinControl, coin_type, useIX))
                {
                    ...
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason += " " + _("Unable to locate enough funds for this transaction that are not equal 100000 CMM.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT100000IFMN) {
                        strFailReason += " " + _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 100000 CMM.");
                    } else {
